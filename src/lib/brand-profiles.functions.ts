...
const brandSchema = z.object({
  ...
  font_primary: z.string().max(80).nullable().optional(),
  font_secondary: z.string().max(80).nullable().optional(),
  font_price: z.string().max(80).nullable().optional(),
...
export const upsertBrandProfileFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const payload = {
      ...
      font_family: data.font_family ?? data.font_primary ?? null,
      font_primary: data.font_primary ?? data.font_family ?? null,
      font_secondary: data.font_secondary ?? null,
      font_price: data.font_price ?? null,
