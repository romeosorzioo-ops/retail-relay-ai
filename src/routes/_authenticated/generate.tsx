...
import { generateContentFn } from "@/lib/content.functions";
...
function GeneratePage() {
  ...
  const gen = useMutation({
    mutationFn: () =>
      generateContentFn({ data: { promotion_id: selected } }),
    onSuccess: (r) => {
      setResult(r);
  ...
  const blocks = result
    ? [
        ...
        },
        {
          ...
          value: result.instagram_post,
        },
        {
          icon: Instagram,
          label: "Story Instagram",
          value: result.instagram_story,
        },
        { icon: Film, label: "Idée de Reel", value: result.reel_idea },
      ]
    : [];
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {blocks.map((b) => (
            <Card key={b.label}>
              ...
              <CardContent>
                <p className="whitespace-pre-wrap text-sm">{b.value}</p>
              </CardContent>
            </Card>
          ))}
        </div>
